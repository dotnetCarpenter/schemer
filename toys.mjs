...
import eq from './lib/eq.mjs'


assert(isAtom([]) === false, "No because [] is just a list.")

assert(car(['a','b','c']) === 'a', "a because a is the first atom of this list.")
assert(eq( car([['a', 'b', 'c'], 'x', 'y', 'z']), ['a', 'b', 'c'] ), "because [a b c] is the first S-expression of this non-empty list.")
...
assert(eq( car([[['hotdogs']], ['and'], ['pickle'], 'relish']), [['hotdogs']]), "The list of the list of hotdogs. [['hotdogs']] is the first S-expression.")
assert(eq(car(car( [[['hotdogs']], ['and']] )), ['hotdogs']), "['hotdogs']")


assert(eq(cdr(['a', 'b', 'c']), ['b', 'c']), "because ['b', 'c'] is the list ['a', 'b', 'c'] without ( car(['a', 'b', 'c'])).")
assert(eq(cdr([['a', 'b', 'c'], 'x', 'y', 'z']), ['x', 'y', 'z']), "['x', 'y', 'z']")
assert(eq(cdr(['hamburger']), []), "[]")
...
assert(eq(car(cdr( [['b'], ['x', 'y'], [['c']]] )), ['x', 'y']), "(x y), because ((x y) ((c))) is (cdr l), and (x y) is the car of (cdr l).")
assert(eq(cdr(cdr( [['b'], ['x', 'y'], [['c']]] )), [[['c']]] ), "(((c))), because ((x y) ((c))) is (cdr l), and (((c))) is  the cdr of (cdr l).")
...
assert(eq( cons('peanut', ['butter', 'jelly']) , [ 'peanut', 'butter', 'jelly' ]), "(peanut butter and jelly), because cons adds  an  atom to  the front of a list.")
assert(eq( cons(['banana', 'and'], [ 'peanut', 'butter', 'jelly' ]) , [ [ 'banana', 'and' ], 'peanut', 'butter', 'jelly' ]), "((banana and) peanut butter and jelly), because cons adds any S-expression to the front of a list.")
assert(eq( cons([['help'], 'this'], ['is', 'very', [['very'], 'to', 'learn']]) , [ [ [ 'help' ], 'this' ], 'is', 'very', [ [ 'very' ], 'to', 'learn' ] ]), "(((help) this) is  very  ((hard) to learn)).")
assert(eq( cons(['a', 'b', ['c']], []), [ [ 'a', 'b', [ 'c' ] ] ]) , "((a b (c))), because  () is a list.")
assert(eq( cons('a', []), ['a'] ), "(a).")
...
assert(eq( cons('a',car([['b'],'c','d'])), ['a','b']), "['a','b']")
assert(eq( cons('a', cdr([['b'],'c','d'])), ['a','c','d']), "['a','c','d']")
...
assert( eq('Harry', 'Harry'), 'True, because (eq? a1  a2) is just another way to ask, "Are a1 and a2 the same non-numeric atom?"')
assert( eq('margaring', 'butter') === false, 'False, since a1 and a2 are different atoms.')
assert( eq.length === 2, "It takes two arguments. Both of them must be non-numeric atoms.")
assert( eq(quote(), quote('strawberry')) === false, "No answer, () and (strawberry) are lists. In practice, lists be arguments of eq?. Two lists are eq? if they are the same list.")
assert( eq(6, 7) === false, 'No answer, 6 and 7 are numbers.')

// The Law of Eq?
// The primitive eq? takes two ar­guments.
// Each must be a non­numeric atom.

assert( eq (car ( quote('Mary','had','a','little','lamb','chop')), 'Mary'), "True, because (car l) is the atom Mary, and the argument a is also the atom Mary.")
assert.throws( () => { eq (cdr ( quote('soured', 'milk')), 'milk') }, "No answer. See The Laws of Eq? and Cdr.")
{
